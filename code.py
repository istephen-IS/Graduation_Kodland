...
@bot.command()
async def sq(ctx, left: int, right: int):
...
async def deteksi(ctx):
    if ctx.message.attachments:
        for attachment in ctx.message.attachments:
            file_name = attachment.filename
            #file_url = attachment.url IF URL
            await attachment.save(f"./images2/{file_name}")
            await ctx.send(detect(input_image=f"./images2/{file_name}", output_image=f"./images2/{file_name}", model_path="yolov3.pt"))
            with open(f'images2/{file_name}', 'rb') as f:
...
@bot.command()
async def reverse(ctx, *, x: str):
    y = x
    await ctx.send(' '.join(y.split()[::-1]))
